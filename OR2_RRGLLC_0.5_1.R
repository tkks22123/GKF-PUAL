...
data11_<-read.csv("F:/ucl/pr/dataset/112/ngood/online_retail_II.csv",header = F)[1:175957,]
data111112<-data11_[,5]
data111_<-rbind(data11_[data111112=="Germany",][1:500,],data11_[data111112=="United Kingdom",][1:500,])
ran_start<-122385
set.seed(ran_start)
...
sk<-sdcc(defpar)
iter_times<-RYL(iter_times,ran_seed,ran_start)
n_32<-2
...
{ ######################
  ...
  X_l <- X_l_org
  X_u <- X_u_org
  #X_P <- X_P_org[-(((cro-1)*s_size/(times_cross-1)+1):(cro*s_size/(times_cross-1))),]
  #X_N <- X_N_org[-(((cro-1)*s_size_n/(times_cross-1)+1):(cro*s_size_n/(times_cross-1))),]
  X_lu <- rbind(X_l,X_u)
  lp  <- lp_org
  lu <- lu_org
  ...
  #Phi_lu <-X_lu%*% solve(KK_M11-1/KK_M22*(KK_M12%*%KK_M21)  )%*%t(X_lu)
  rbf <- rbfdot(sigma = 1)
  ...
  {  
    #####update alpha
     ...
     #Phi_lt[tr_a,]<- -Phi_lt[tr_a,]
     #Phi_ut[sfk,]<- -Phi_ut[sfk,]
     #Phi_ut[dq.c,]<- -Phi_ut[dq.c,]
     #one_lt[tr_a]<- -one_lt[tr_a]
     # one_ut[sfk] <- -one_ut[sfk]
     #one_ut[dq.c]<- -one_ut[dq.c]
