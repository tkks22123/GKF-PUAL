...
inte<-function(a=c(length(data12),n_f),def=defpar)
{if((a[1]==def[1]-1)&(a[2]==def[2])){1}
 else{0}
}

fyj<-function(tsym,cdmn)
{
  if(length(tsym)>length(cdmn))
  {return(tsym)}
  if(length(tsym)<length(cdmn))
  {return(cdmn)}
  if(length(tsym)==length(cdmn))
...
lambda_1<-fyj(1,1)
#Duplicate strategy for ADMM
dup_AD<-function(A,q=sk)
{
  t2<-sample(1:length(A))[1:(q*length(A))]
  A[t2]<--A[t2]
  A
}
#The S/C function
...
#initial of the dual parameters for ADMM
sdcc<-function(defpar=defpar)
{sqrt(defpar[3])-1.5+runif(n=1,min=-defpar[4]^3,max=defpar[4]^3)
}
...
Qu<-function(mu
,XX_4=matrix(0,nrow(X_lu),nrow(X_lu)),n_ccol=ncol(X_lu))
{ n_rrow<-0.5*(n_ccol^2+n_ccol)
  n_re=ncol(XX_4)
  XX_re_all<-matrix(0,nrow=0,ncol=n_rrow+n_ccol)
  XX_re_all2<-matrix(0,nrow=nrow(XX_4),ncol=ncol(XX_4))
  XX_re<-as.vector(XX_4[,1])
  XX_re_1<-cbind(XX_re,XX_re)
  XX_re_all<-rbind(XX_re_all)
  return(XX_re_all2)
}
