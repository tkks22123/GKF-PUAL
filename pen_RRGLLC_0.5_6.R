...
data11_<-read.csv("F:/ucl/pr/dataset/112/ngood/pendigits.csv",header = F)
data111112<-data11_[,17]
data111_<-rbind(data11_[data111112==1,][1:200,],data11_[data111112==8,][1:200,],data11_[data111112==4,][1:400,])
ran_start<-12235
set.seed(ran_start)
...
ran_seed<-ran_start# (1163124)

for(ran_seed in (13997))
{set.seed(ran_seed)
  ranran<-sample(1:nrow(data111_))
  data1<-data111_[ranran,]
  data11<-data1[,c(1:16,18:49)]
  data12<-data1[,17]
  a2=c(50,40,-75,-10)
...
mu_3_star<-0.001
Phi_test<-TX2
for(lambda_1 in c(0.1))
for(lambda_2 in c(0.13))
for(C_1 in c(1)) 
for(C_2 in c(0.025))
for(mu_1 in 0.01)
  {{{{{
