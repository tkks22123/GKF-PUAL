...
data111_<-read.table("F:/ucl/pr/dataset/112/ngood/seeds_dataset.txt",header = FALSE,sep =" ")
ran_start<-122385
set.seed(ran_start)
...
{set.seed(ran_seed)
  ...
  data12<-data1[,8]
  data1<-data1[data12==1|data12==2,]
  data11<-(data1[,c(1:7,9:23)])
  data12<-data1[,8]
  data12[data12==1] <- 1
  data12[data12==2] <- -1
  data12<-as.numeric(data12)
  
  defpar<-c(119,6,2.56,0.2714418)
  fre_label=0.5
  p_trte=0.7
  re_sig=0.1
